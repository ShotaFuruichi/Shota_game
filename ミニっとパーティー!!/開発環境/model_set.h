...
#define MAX_MODEL_TYPE		(50)	// モデルの種類の数
...
typedef struct
{
	...
	Model modelInfo[MAX_MODEL_TYPE];		// モデル情報
	ModelSetInfo modelSet[MAX_MODEL];		// モデル設置
}ModelSet;
...
void TexModel(int ModelSet);										// モデルのテクスチャ
void VecModel(int ModelSet);										// モデルの頂点座標

void VecModelSet(int nNumModelSet);									// モデルの頂点座標

void SetModel(int type, D3DXVECTOR3 pos, D3DXVECTOR3 rot, int nCollition);	// モデルセット

bool CollisionModelSet(D3DXVECTOR3 *pPos, D3DXVECTOR3 *pPosOld,
	D3DXVECTOR3 *pMove,D3DXVECTOR3 *pvtxMin, D3DXVECTOR3 *pvtxMax, int nIdxShadow, D3DXVECTOR3 *PosShadow);	// 当たり判定

ModelSet *GetModelSet(void);											// モデル情報の取得
