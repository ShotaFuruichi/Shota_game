...
// マクロ定義
//-------------------------------------------------------------------
#define TIMER_X					(660.0f)		// マルチモードのタイマーの中心座標(X)
#define TIMER_Y					(60.0f)			// マルチモードのタイマーの中心座標(Y)

#define TIMER_WIDTH				(40.0f)			// タイマーの幅
#define TIMER_HEIGHT			(64.0f)			// タイマーの高さ
#define TIMER_SPACE				(35.0f)			// タイマーの数字を配置する間隔

#define MAX_PATTERN				(10)			// アニメーションパターンNo.の最大数
#define TEXTURE_X				(10)			// アニメーションパターンNo.の横の最大数
#define TEXTURE_Y				(1)				// アニメーションパターンNo.の縦の最大数

#define MAX_TIMER				(2)				// スコアテクスチャの最大値
#define TIME					(60)			// タイムの設定
...
HRESULT InitTimer(void)
{
	...
	// 変数の初期化
	g_posTimer = D3DXVECTOR3(TIMER_X, TIMER_Y, 0.0f);
	g_nTimer = TIME;								// 時間			
