...
// マクロ定義
//-----------------------------------------------------------------
#define MAX_LOADSCREEN  (6)							// 使うテクスチャの最大数
#define MAX_TEXTURE		(6)
#define MAX_BG			(1)							// 背景の最大数
#define MAX_SELECT		(3)							// マーカーの最大値
#define MAX_PAGE		(3)							// 画像の最大数
...
#define SELECT_X		(595.0f)					// マーカーの中心座標(X)
...
HRESULT InitLoadScreen(void)
{
	...
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/screen_oe002.png", &g_pTextureLoadScreen[2]);
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/screen_se.png", &g_pTextureLoadScreen[3]);
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/screen_ge.png", &g_pTextureLoadScreen[4]);
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/press_A.png", &g_pTextureLoadScreen[5]);

	// 頂点バッファの生成
	if (FAILED(pDevice->CreateVertexBuffer(
		sizeof(VERTEX_2D) * MAX_VERTEX * MAX_LOADSCREEN, // 確保するバッファサイズ
		D3DUSAGE_WRITEONLY,
	...
	for (int nCnt = 0; nCnt < MAX_LOADSCREEN; nCnt++)
	{
		// 頂点座標の設定
	...
	{// プレスOKを押してないとき
	 // 画面遷移
		...
		if (pStick[STICKTRIGGER_LEFT] == true ||
			...
			GetJoypadTrigger(PLAYER_1, JPINFO_CANCEL) == true)
		{// 左に移動するとき
			if (g_nCntSelect != 0)
			{
